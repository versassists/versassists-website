...
export default function HeaderClient({
  ...
  const logoSrc = logoUrl || "/logos/versassist png logo.avif";
    ...
    >
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          <Link href="/" className="flex items-center" aria-label={`${companyName} home`}>
            <Image
              src={logoSrc}
              alt={companyName}
              width={156}
      ...
      <div className={`lg:hidden transition-all duration-300 overflow-hidden ${isOpen ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="bg-white border-t shadow-lg px-6 py-6 space-y-1">
          ...
          <a href={calendlyUrl} target="_blank" rel="noopener noreferrer" className="block px-4 py-3 mt-3 text-center rounded-full bg-primary text-white text-sm font-semibold">
            Book a Call
          </a>
        </div>
      </div>
